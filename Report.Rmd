...
The following table shows the coefficients of the logistic regression performed with the covariates time, dose and PCV to predict the missings. The model was:

$$logit(p) = \beta_{0}  + \beta_1PCV + \beta_2 doseMedium + \beta_3 doseHigh + \beta_{4}time$$

All the coefficients have a lot of variation depending on the values of the coefficient of the PCV (Table \ref{pcvmissings}). The missings then, depend a lot on the covariates and also on the values of PCV. Other sensitivity analyses were done with the categorical PCV, leading to similar results (Table \ref{catpcv}). The model used is:
...
xtaula <- xtable(cbind(Beta1=a, t(taula)), caption="Regression coefficients and SD given the coefficient for the PCV.", label="pcvmissings")
...
xtaula <- xtable(cbind(Beta1=a, t(taula)), caption="Regression coefficients and SD given the coefficient for the PCV.", label="catpcv")
print(xtaula, comment=F, include.rownames=F)
```
