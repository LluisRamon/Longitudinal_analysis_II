...
Extremely high (or low) values were obtained because there are only twice as much observations than parameters to estimates. Besides in our dataset one can notice that the zeros are mainly at time 1 and for the low dosage and the one at time 3 for the high dosage. This is a complete separation problem. It means that values of a variable completely predict the value of the outcome variable. For such an issue the likelihood does not have a maximum for the corresponding variable and so the estimates are very large. 
...
* A solution to the problem of separation in logistic regression. http://www.ncbi.nlm.nih.gov/pubmed 
