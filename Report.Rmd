...
The relative low number of observations in the dataset makes difficult to generalize the results of the study. A lot of assumptions and issues arised when we analyzed the data. In order to verify whether or not those assumptions can be validated we would have needed more information on the dataset.

In order to further analyse the data some improvements are possible. 

In GEE modeling for small sample, Jacknife estimators are better than sandwhich estimator as in (Højsgaard, 2006) \ref{sec:Bibliography}

A new factor High dose against a combined Low and Medium dose could be done.
