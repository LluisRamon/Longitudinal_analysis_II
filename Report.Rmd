...
Perfect separation occurs in logistic regression when a covariate or a combination of them almost completely separates the value of the outcome variable. In this case the likelihood does not have a maximum for the corresponding variable and so the estimates became very large. It frequently occurs with small sample. A way to detect it is when the model has very big coefficients and standard errors. It is fully described in (Heinze 2002) \ref{art:Heinze 2002}. 
