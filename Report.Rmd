...
for(i in 1:10){
  ...
  vaca <- vaca[c(which(vaca$dose==names(tbord)[1]),
                 which(vaca$dose==names(tbord)[2]),
                 which(vaca$dose==names(tbord)[3])),]
  cows[cows$id==i,] <- vaca
}
...
image(x=1:10, y=1:9, t(colors*b), yaxt="n", ylab="Times", xlab="ID", col=c("white", heat.colors(3)[3:1]))
abline(v=1:10-0.5)
abline(h=1:10-0.5)
axis(2, at=1:9, rep(1:3, 3))
```
