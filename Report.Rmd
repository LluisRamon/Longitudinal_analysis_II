...
xtaula <- xtable(xx, label="modelgee", caption="Coefficients, SD and p-values from the GEE model. Alpha estimation (SD) = 0.171 (0.0413).")
...
```{r classifcation_GEE, results='asis'}
...
xtaula <- xtable(xx, caption="Crosstable with the predicted data with the model and the original data.", label="pred-vs-original")
print(xtaula, comment=FALSE)
...
Perfect separation occurrs in logistic regression when a covariate or a combination of them almost completely separates the value of the outcome variable. In this case the likelihood does not have a maximum for the corresponding variable and so the estimates became very large. It frequently ocurrs with small sample. A way to detect it is when the model has very big coefficients and standard errors. It is fully described in (Heinze 2002) \ref{art:Heinze 2002}. 

Our model has the previous conditions, so we consider that this model it is not appropiate and it is suggested to switch for a model that takes into account this issue. Several alternative models are described in section Further research \ref{sec:Further_research}.
...
and 0.007, respectively). Actually, there are more missings in low and medium dose
than in high dose, and also, there are more missings as time increases. 
Missings in the PCV are not produced completely at random, then.
...
missings have the first missing after being unhealthy, it would be logic to 
...
(missing PCV's were omited), the results obtained could be far from the reality 
...
* A solution to the problem of separation in logistic regression. http://www.ncbi.nlm.nih.gov/pubmed/12210625 \label{art:Heinze 2002}
