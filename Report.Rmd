...
orfixed <- as.data.frame(c("intercept"=exp(coef(finalModel)$id[[1]][1]),"doseM"=exp(coef(finalModel)$id[[2]][1]),"doseH"=exp(coef(finalModel)$id[[3]][1])))

orrandom <- exp(c(coef(finalModel)$`dose:id`[[4]],coef(finalModel)$id[[4]]))

print(xtable(orfixed))
```

TODO(Mathieu): modify the xtable

The values of the estimates of the model 
